...
def get_all_stocks():
    try:
        ...
        df = pro.daily(trade_date=today, fields='ts_code', limit=1000)
        ...
        pass
    return ['000001.SZ', '000002.SZ', '000858.SZ', '002415.SZ', '000333.SZ',
            '600519.SH', '600036.SH', '601318.SH', '600900.SH', '601166.SH']
...
def get_stock_data(code, trade_date):
    try:
        ...
        for i in range(len(last_6)):
            if last_6['close'].iloc[i] <= last_6['ma10'].iloc[i]:
                return None
        ...
        # 4. 10日内最低价不跌破MA20（新条件）
        for i in range(len(last_10)):
            if last_10['low'].iloc[i] < last_10['ma20'].iloc[i]:
                return None
...
def send_email(results, date_str):
    if not results:
        ...
        for r in results:
            ...
            body += f"  收盘：{r['close']:.2f} | MA10：{r['ma10']:.2f} | MA20：{r['ma20']:.2f}\n"
